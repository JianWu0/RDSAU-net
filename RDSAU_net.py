...
###
       self.encoder1 = nn.Conv2d(3, 64, 3, stride=1, padding=1)  # 通道数3>>16
       self.encoder2 = nn.Conv2d(64, 128, 3, stride=1, padding=1)
       self.ebn1 = nn.BatchNorm2d(64)  # Batch Normalization的目的是使我们的一批（ Batch)feature map满足均值为o，方差为1的分布规律。
       self.ebn2 = nn.BatchNorm2d(128)  # 32代表通道数

       self.decoder3 = nn.Conv2d(256, 64, 3, stride=1, padding=1)
...
###
        x15 = self.decoder3(x14)
        x15 = self.dbn3(x15)
        x16 = self.up_conv4(x15, x1)
        x17 = self.decoder4(x16)
        x17 = self.dbn4(x17)
###
